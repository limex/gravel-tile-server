...
>This is the attempt to get the gravel overlay in a local container. Seems like this is not for the overlay, but the normal map for the CX Berlin Site.
...
> following adaptions are from https://github.com/Overv/openstreetmap-tile-server/edit/master/README.md. plus local pathes added.
...
```
docker run \
    -v /Users/guntherbosch/Sources/gravel-tile-server/osm.pbf/austria-latest.osm.pbf:/data/region.osm.pbf \
    -v osm-data:/data/database/ \
    overv/openstreetmap-tile-server \
    import
```

> Attempt to add gravel style instead of carto (not working): 

```
docker run \
    -v /Users/guntherbosch/Sources/gravel-tile-server/osm.pbf/austria-latest.osm.pbf:/data/region.osm.pbf \
    -v /Users/guntherbosch/Sources/gravel-cartocss-style:/data/style/ \
    -v osm-data:/data/database/ \
    overv/openstreetmap-tile-server \
    import
```
...
https://api.mapbox.com/styles/v1/limex67/ckchx6lz72i9u1iphwz9zam0i/tiles/256/18/142344/92138
http://localhost:8080/tile/18/142344/92138.png
